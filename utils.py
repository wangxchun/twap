...
import custom_market as custom
...
def plot_efficient_frontier(tr_risk = 1e-6):
    ...
    # Set X axis formatter to display in billions (B)
    xNumFmt = mticker.StrMethodFormatter('{x:,.0f}B')

    # Convert V values to millions for plotting
    V_in_billions = V / 1e9
    tr_V_in_billions = tr_V / 1e9

    # Plot E vs V and use U for the colorbar    
    cm = plt.cm.get_cmap('gist_rainbow')    
    sc = plt.scatter(V_in_billions, E, s=20, c=U, cmap=cm)
    ...
    plt.xlabel('Variance of Shortfall (in Billions)')
    plt.ylabel('Expected Shortfall')

    # Get the annotation label and the correction factors
    an_st, xcrf, ycrf, scrf = get_crfs(tr_risk)

    # Plot the annotation in the above plot, converting the x-coordinate to millions
    plt.annotate(an_st, xy=(tr_V_in_billions, tr_E), xytext=(tr_V_in_billions * xcrf, tr_E * ycrf), color='w', size='large',
